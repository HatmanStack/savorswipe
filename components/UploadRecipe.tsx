...
const UploadImage = ({ onImageUploaded }: { onImageUploaded: (base64Image: string) => void }) => { // Define the component with props
  ...
  const selectImage = async (onImageUploaded: (base64Image: string) => void) => { // Accept onImageUploaded as a parameter
    ...
    if (!result.canceled) {
      ...
      if (imageUri) { 
        ...
        if (resizedImage) {
          onImageUploaded(resizedImage); // Call the prop function with the resized image
  ...
  return (
    // You can add a button or any UI element to trigger selectImage
    <></>
  );
};
