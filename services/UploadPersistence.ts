...
export class UploadPersistence {
  ...
  static async clear(): Promise<void> {
    try {
      ...
      if (__DEV__) {
        console.warn('Failed to clear upload queue:', error)
      }
      // Don't throw - persistence is optional
    }
  }
  ...
  static async getCompletionFlags(jobIds: string[]): Promise<Map<string, CompletionFlag>> {
    ...
    for (const jobId of jobIds) {
      try {
        ...
        if (__DEV__) {
          console.warn(`Failed to fetch completion flag for ${jobId}:`, error)
        }
  ...
  static async deleteCompletionFlag(jobId: string): Promise<void> {
    try {
      ...
      if (__DEV__) {
        console.warn(`Failed to delete completion flag for ${jobId}:`, error)
      }
    }
  }
}
