...
const filterInternalKeys = ([key]: [string, unknown]) =>
  !["kind", "value", "items", "ingredients", "steps", "sections"].includes(key);
...
function RecipeDetails({ currentRecipe }: { currentRecipe: Recipe | null }) {
  ...
  function getIngredientEntries(
    ing: NonNullable<Recipe["Ingredients"]>,
  ): [string, unknown][] {
    ...
    if ("kind" in ing && ing.kind === "flat" && "ingredients" in ing) {
      return Object.entries(
        (ing as { ingredients: Record<string, string> }).ingredients,
      );
    }
    // If normalized 'sectioned' format, use the nested 'sections' field
    if ("kind" in ing && ing.kind === "sectioned" && "sections" in ing) {
      return Object.entries(
        (ing as { sections: Record<string, Record<string, string>> }).sections,
      );
    }
  ...
  function getDirectionEntries(
    dir: NonNullable<Recipe["Directions"]>,
  ): [string, unknown][] {
    ...
    if ("kind" in dir && dir.kind === "sectioned" && "sections" in dir) {
      return Object.entries(
        (dir as { sections: Record<string, Record<string, string>> }).sections,
      );
    }
    // Raw format - filter out any 'kind' key that might have leaked in
  ...
  return (
    <ScrollView
      style={[styles.scrollview, { padding: dimensions.width * 0.02 }]}
    >
      <ThemedView style={styles.title}>
        ...
        {currentRecipe.Description && (
          <Collapsible title="Description">
            {Array.isArray(currentRecipe.Description) ? (
              ...
              <ThemedText>
                {decodeUnicode(currentRecipe.Description)}
              </ThemedText>
            )}
          </Collapsible>
        )}
        ...
        !Array.isArray(currentRecipe.Ingredients) ? (
          <Collapsible title="Ingredients">
            {getIngredientEntries(currentRecipe.Ingredients).map(
              ([key, value], index) => {
                ...
                if (typeof value === "object" && !Array.isArray(value)) {
                      ...
                      {Object.entries(value as Record<string, string>).map(
                        ([ingredient, amount], ingredientIndex) => (
                          <ThemedText key={ingredientIndex}>
                            {decodeUnicode(ingredient)}:{" "}
                            {amount ? decodeUnicode(amount) : ""}
                          </ThemedText>
                        ),
                      )}
                    </Collapsible>
                  ...
                  return (
                    <ThemedText key={index}>
                      {decodeUnicode(key)}:{" "}
                      {value ? decodeUnicode(value as string) : ""}
                    </ThemedText>
                  );
                }
              },
          ...
          <Collapsible title="Ingredients">
            {Array.isArray(currentRecipe.Ingredients) ? (
              ...
              <ThemedText>
                {decodeUnicode(currentRecipe.Ingredients as string)}
              </ThemedText>
            )}
          </Collapsible>
        )}

        {typeof currentRecipe.Directions === "object" &&
        !Array.isArray(currentRecipe.Directions) ? (
          ...
          <Collapsible title="Directions">
            {Array.isArray(currentRecipe.Directions) ? (
              ...
              <ThemedText>
                {decodeUnicode(currentRecipe.Directions as string)}
              </ThemedText>
            )}
          </Collapsible>
        )}
