...
async function installStubs(page: Page) {
  ...
  await page.route(
    `${FAKE_API}/upload/status/${FIXTURE_JOB_ID}`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
...
test("app boot happy path", async ({ page }) => {
  ...
  // Register the response wait BEFORE navigating. The recipes fetch is issued
  // from RecipeProvider's mount effect, which under the Expo 57 / React 19
  // runtime can resolve before page.goto() settles on 'load'. Awaiting
  // waitForResponse after goto() therefore races and can miss the response.
