...
class TestEmbeddingGenerator(unittest.TestCase):
    ...
    def test_recipe_to_text_nested_dict(self):
        ...
        recipe = {
            "Title": "Layered Cake",
            "Ingredients": {
                "For the Crust": {"graham crackers": "2 cups", "butter": "1/2 cup"},
                "For the Filling": {"cream cheese": "8 oz", "sugar": "1 cup"},
            },
        }
        text = EmbeddingGenerator.recipe_to_text(recipe)
    ...
    def test_generate_embedding_success(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"data": [{"embedding": self.mock_embedding}]}
    ...
    def test_model_reads_from_config(self):
        """Test the embedding model comes from config, not a hardcoded literal."""
        self.assertEqual(EmbeddingGenerator.MODEL, config.OPENAI_EMBEDDING_MODEL)
        self.assertEqual(config.OPENAI_EMBEDDING_MODEL, "text-embedding-3-small")

    @patch("embedding_generator.SESSION.post")
    def test_generate_embedding_uses_configured_model(self, mock_post):
        """Test that the API request body carries the configured model."""
        mock_response = Mock()
        mock_response.json.return_value = {"data": [{"embedding": self.mock_embedding}]}
        mock_response.raise_for_status = Mock()
        mock_post.return_value = mock_response
        ...
        self.assertEqual(request_data["model"], config.OPENAI_EMBEDDING_MODEL)
    ...
    def test_generate_recipe_embedding(self, mock_post):
        ...
        recipe = {"Title": "Test Recipe", "Ingredients": ["flour", "sugar"]}
